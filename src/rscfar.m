...
alpha_factor = 4.5;     % Threshold scaling factor

% Divide range into slices (e.g., 4 slices)
numSlices = 4;
sliceSize = floor(numADCSamples / numSlices);

cfarMask = zeros(1, numADCSamples);

for s = 1:numSlices
    startIdx = (s - 1) * sliceSize + 1;
    endIdx = min(s * sliceSize, numADCSamples);
    
    sliceData = rangeProfile(startIdx:endIdx);
    sliceMean = mean(sliceData);
    
    for i = startIdx + numTrainingCells + numGuardCells : endIdx - numTrainingCells - numGuardCells
        ...
        threshold = alpha_factor * max(sliceMean, mean(trainingCells));
...
% Subplot 1: Range Profile with RS-CFAR Detections
subplot(1,1,1);
...
title('Range Profile with RS-CFAR Detection');
xlabel('Range (m)');
ylabel('Amplitude (dB)');
...
disp('RS-CFAR Object Detection with Subplot Output Completed!');
