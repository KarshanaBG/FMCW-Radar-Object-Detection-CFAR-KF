...
alpha_factor = 5.0;     % Threshold scaling factor

cfarMask = zeros(1, numADCSamples);

for i = numTrainingCells + numGuardCells + 1 : numADCSamples - numTrainingCells - numGuardCells
    leadingCells = rangeProfile(i + numGuardCells + 1 : i + numGuardCells + numTrainingCells);
    trailingCells = rangeProfile(i - numTrainingCells - numGuardCells : i - numGuardCells - 1);

    % Greatest-Of Selection
    greatestAvg = max(mean(leadingCells), mean(trailingCells));
    
    % CFAR Threshold
    threshold = alpha_factor * greatestAvg;
...
title('Range Profile with CAGO-CFAR Detection');
xlabel('Range (m)');
ylabel('Amplitude (dB)');
