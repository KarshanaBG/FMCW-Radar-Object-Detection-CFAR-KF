...
for i = (numTrainingCells + numGuardCells + 1):(numADCSamples - numTrainingCells - numGuardCells)
    ...
    % Adaptive threshold using mean and variance of training region
    meanTraining = mean(trainingCells);
    stdTraining = std(trainingCells);
    
    % Dynamic threshold = mean + alpha * standard deviation
    threshold = meanTraining + alpha_factor * stdTraining;
...
title('Range Profile with AC-CFAR Detection');
xlabel('Range (m)');
ylabel('Amplitude (dB)');
...
disp('✅ AC-CFAR Object Detection with Subplot Output Completed!');
